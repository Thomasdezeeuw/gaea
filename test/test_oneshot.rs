...
pub fn test_tcp_edge_oneshot() {
    ...
    poll.register(&mut l, Token(0), Ready::READABLE, PollOpt::LEVEL).unwrap();

    // Connect a socket, we are going to write to it
    let mut s1 = TcpStream::connect(l.local_addr().unwrap()).unwrap();
    poll.register(&mut s1, Token(1), Ready::WRITABLE, PollOpt::LEVEL).unwrap();

    wait_for(&mut poll, &mut events, Token(0));

    // Get pair
    let (mut s2, _) = l.accept().unwrap();
    poll.register(&mut s2, Token(2), Ready::READABLE, PollOpt::EDGE | PollOpt::ONESHOT).unwrap();
    ...
    for byte in b"foo" {
        ...
        poll.reregister(&mut s2, Token(2), Ready::READABLE, PollOpt::EDGE | PollOpt::ONESHOT).unwrap();
...
fn wait_for(poll: &mut Poll, events: &mut Events, token: Token) {
    loop {
        ...
        let cnt = events.into_iter().filter(|e| e.token() == token).count();
