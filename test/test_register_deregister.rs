...
impl TestHandler {
    ...
    fn handle_read(&mut self, poll: &mut Poll, token: Token) {
        match token {
            ...
            CLIENT => {
                ...
                self.state = 1;
                poll.reregister(&mut self.client, CLIENT, Ready::WRITABLE, PollOpt::LEVEL).unwrap();
            }
    ...
    fn handle_write(&mut self, poll: &mut Poll, token: Token) {
        ...
        self.state = 2;
        poll.deregister(&mut self.client).unwrap();
        poll.deregister(&mut self.server).unwrap();
    }
}
...
pub fn test_register_deregister() {
    ...
    poll.register(&mut client, CLIENT, Ready::READABLE, PollOpt::LEVEL).unwrap();
...
pub fn test_register_empty_interest() {
    ...
    let mut sock = TcpListener::bind(addr).unwrap();

    poll.register(&mut sock, Token(0), Ready::empty(), PollOpt::EDGE).unwrap();
    ...
    poll.register(&mut client, Token(1), Ready::empty(), PollOpt::EDGE).unwrap();
