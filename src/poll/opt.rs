...
///
/// [registering]: struct.Poll.html#method.register
...
/// oneshot flag. This tells `Poll` to disable the associated [`Evented`] after
/// the event is returned from [`Poll.poll`], note that *disabled* and
/// *deregistered* are not the same thing. Subsequent calls to [`Poll.poll`]
...
///
/// Assuming the socket was registered with `Poll` with the *oneshot* option,
/// then the call to [`Poll.poll`] in step 7 would block. This is because,
/// oneshot tells `Poll` to disable events for the socket after returning an
/// event.
///
